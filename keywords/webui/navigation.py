...
class Navigation:
    ...
    def navigate_to(cls, location: str, header: str, location2: str = "") -> None:
        ...
        if COM.is_visible(xpaths.common_xpaths.button_field('close-error-dialog'), 1):
            print(f'@@@@@@@ ERROR_DIALOG_LEFT_OPEN-{create_timestamp()}')
            take_screenshot(f'ERROR_DIALOG_LEFT_OPEN-{create_timestamp()}')
            COM.click_error_dialog_close_button()
            COM.click_button('close')
        if COM.is_visible(xpaths.common_xpaths.button_field('do-not-start'), 1):
            print(f'@@@@@@@ Start_Service_Dialog_Open-{create_timestamp()}')
            take_screenshot(f'Start_Service_Dialog_Open-{create_timestamp()}')
            COM.click_button('do-not-start')
        if COM.is_visible(xpaths.common_xpaths.button_field('do-not-restart'), 1):
            print(f'@@@@@@@ Restart_Service_Dialog_Open-{create_timestamp()}')
            take_screenshot(f'Restart_Service_Dialog_Open-{create_timestamp()}')
