...
@allure.feature('Read Only Admin', 'Rsync Tasks', 'Data Protection', 'Users', 'Permissions')
...
class Test_Read_Only_Admin_Data_Protection_Rsync_Tasks:
    ...
    def test_read_only_admin_can_view_the_configured_rsync_task(self):
        ...
        - Verify that the rsync task is visible on the Rsync Task card as a read-only admin.
        """
    ...
    def test_read_only_admin_can_access_rsync_task_list_view_from_the_data_protection_page(self):
        """
        ...
        - Verify that the rsync task listed and visible on the page as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
        Data_Protection.click_the_rsync_task_header_link()
        assert Rsync_Task.assert_rsync_task_page_header()
    ...
    def test_read_only_admin_cannot_add_a_rsync_task_from_the_card(self):
        ...
        - Navigate to the data protection page.
        - Verify the add button on Rsync Task card is locked and not clickable as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
        assert Data_Protection.assert_add_rsync_task_button_is_locked_and_not_clickable() is True

    @allure.tag("Create")
    @allure.story('Verify That The Read-Only Admin Cannot Add A Rsync Task From The Page')
    def test_read_only_admin_cannot_add_a_rsync_task_from_the_page(self):
        """
        This test verifies that the read-only admin cannot add a rsync task from the page.
        - Navigate to the data protection page.
        - Click on the Rsync Task header link.
        - Verify the Add button on the Rsync Task page is locked and not clickable as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
        Data_Protection.click_the_rsync_task_header_link()
        assert Rsync_Task.assert_rsync_task_page_header()
        assert Rsync_Task.assert_add_rsync_task_button_is_locked_and_not_clickable() is True
    ...
    def test_read_only_admin_cannot_modify_a_rsync_task_from_the_card(self):
        ...
        - verify the edit panel header is visible and the header is read-only as a read-only admin.
        - verify the save button is locked and not clickable as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
    ...
    def test_read_only_admin_cannot_modify_a_rsync_task_from_the_page(self):
        """
    ...
    def test_read_only_admin_cannot_delete_a_rsync_task_from_the_card(self):
        ...
        - Navigate to the data protection page.
        - Verify the delete button of a Rsync Task on Rsync Task card is locked and not clickable as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
        assert Data_Protection.assert_delete_rsync_task_button_is_locked_and_not_clickable('/mnt/tank') is True

    @allure.tag("Delete")
    @allure.story('Verify That The Read-Only Admin Cannot Delete A Rsync Task From The Page')
    def test_read_only_admin_cannot_delete_a_rsync_task_from_the_page(self):
        """
        This test verifies that the read-only admin cannot delete a rsync task from the page.
        - Navigate to the data protection page.
        - Click on the Rsync Task header link.
        - Verify the Delete button of a Rsync Task on the Rsync Task page is locked and not clickable as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
        Data_Protection.click_the_rsync_task_header_link()
        assert Rsync_Task.assert_rsync_task_page_header()
        assert Rsync_Task.assert_delete_rsync_task_button_is_locked_and_not_clickable('/mnt/tank') is True
    ...
    def test_read_only_admin_cannot_enable_and_disable_rsync_tasks(self):
        ...
        - Navigate to the data protection page.
        - Verify the Enable and Disable button on the Rsync Task card is locked and not clickable as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
        assert Data_Protection.assert_enable_rsync_task_toggle_is_locked_and_not_clickable('/mnt/tank') is True
    ...
    def test_read_only_admin_cannot_run_a_rsync_task_from_the_card(self):
        ...
        - Navigate to the data protection page.
        - Verify the Run button on the Rsync Task card is locked and not clickable as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
        assert Data_Protection.assert_run_rsync_task_button_is_locked_and_not_clickable('/mnt/tank') is True

    @allure.tag("Update")
    @allure.story('Verify That The Read-Only Admin Cannot Run A Rsync Task From The Page')
    def test_read_only_admin_cannot_run_a_rsync_task_from_the_page(self):
        """
        This test verifies that the read-only admin cannot run a rsync task from the page.
        - Navigate to the data protection page.
        - Click on the Rsync Task header link.
        - Verify the Run button on the Rsync Task page is locked and not clickable as a read-only admin.
        """
        Navigation.navigate_to_data_protection()
        assert Data_Protection.assert_rsync_task_card_header_is_visible() is True
        Data_Protection.click_the_rsync_task_header_link()
        assert Rsync_Task.assert_rsync_task_page_header()
        assert Rsync_Task.assert_run_rsync_task_button_is_locked_and_not_clickable('/mnt/tank') is True
